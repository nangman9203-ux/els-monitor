...
if st.session_state.all_items is not None:
    ...
    # 각 종목의 원화 환산 금액 계산 (USD는 공시 기재 환율 사용)
    def get_krw_amount(item):
        """종목의 원화 환산 모집총액 반환"""
        if item.offering_total is None:
            return 0
        if item.currency == "KRW":
            return item.offering_total
        elif item.currency == "USD":
            if item.fx_rate:
                return int(item.offering_total * item.fx_rate)
            else:
                # 환율 파싱 실패 시 기본값 1,400원 적용
                return int(item.offering_total * 1400)
        return 0
    
    # 통합 원화 환산 모집총액
    total_offering_krw_equiv = sum(get_krw_amount(x) for x in woori_all)
    
    # 원 통화별 원금 (표시용)
    total_offering_krw_orig = sum(
        (x.offering_total or 0) for x in woori_all if x.currency == "KRW"
    )
    total_offering_usd_orig = sum(
        (x.offering_total or 0) for x in woori_all if x.currency == "USD"
    )
    
    # USD 종목 환율 파싱 여부 확인
    usd_items = [x for x in woori_all if x.currency == "USD"]
    usd_with_fx = [x for x in usd_items if x.fx_rate]
    ...
    c5.metric(
        "모집총액 (원화환산)",
        f"{total_offering_krw_equiv / 100_000_000:,.0f}억원" if total_offering_krw_equiv else "-"
    )
    
    # USD 원화환산 상세
    if usd_items:
        if len(usd_with_fx) == len(usd_items):
            # 모든 USD 종목 환율 파싱 성공
            fx_rates = [x.fx_rate for x in usd_with_fx]
            min_fx, max_fx = min(fx_rates), max(fx_rates)
            if min_fx == max_fx:
                fx_info = f"{min_fx:,.2f}원/USD"
            else:
                fx_info = f"{min_fx:,.2f}~{max_fx:,.2f}원/USD"
            st.caption(
                f"💵 USD 원금: ${total_offering_usd_orig:,} · "
                f"적용환율: {fx_info} (공시 기재 환율)"
            )
        else:
            # 일부 환율 파싱 실패
            missing = len(usd_items) - len(usd_with_fx)
            st.caption(
                f"💵 USD 원금: ${total_offering_usd_orig:,} · "
                f"⚠️ {missing}개 종목 환율 파싱 실패 — 기본 환율 1,400원 적용"
            )
    ...
    if woori:
        ...
        for x in woori:
            key = x.issuer_short
            krw_equiv_by_issuer[key] = krw_equiv_by_issuer.get(key, 0) + get_krw_amount(x)
        
        # 통화별 종목수도 집계 (참고용)
        krw_cnt_by_issuer = Counter(x.issuer_short for x in woori if x.currency == "KRW")
        usd_cnt_by_issuer = Counter(x.issuer_short for x in woori if x.currency == "USD")
        
        summary_data = []
        for issuer, cnt in sorted(by_issuer.items(), key=lambda x: x[1], reverse=True):
            krw_equiv = krw_equiv_by_issuer.get(issuer, 0)
            krw_n = krw_cnt_by_issuer.get(issuer, 0)
            usd_n = usd_cnt_by_issuer.get(issuer, 0)
            
            # 통화 구성 표시
            cur_parts = []
            if krw_n > 0: cur_parts.append(f"KRW {krw_n}")
            if usd_n > 0: cur_parts.append(f"USD {usd_n}")
            cur_str = " / ".join(cur_parts)
            
            summary_data.append({
                "발행사": issuer,
                "종목수": cnt,
                "통화": cur_str,
                "모집총액(억원)": f"{krw_equiv / 100_000_000:,.0f}" if krw_equiv else "-",
            })
